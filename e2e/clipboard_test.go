...
func TestBracketedPasteMultilineCreatesAttachment(t *testing.T) {
	...
	// Paste content with 3+ newlines (should become attachment).
	multilineContent := "line 1\nline 2\nline 3\nline 4"
	require.NoError(t, term.Paste(multilineContent))
	time.Sleep(500 * time.Millisecond)

	// Multi-line paste should create an attachment indicator.
	// Look for the paste_*.txt filename pattern that indicates attachment was created.
	output := term.Output()
	require.True(t,
		strings.Contains(output, "paste_") || strings.Contains(output, ".txt"),
		"Expected multi-line paste to create attachment. Output: %s", output)
}

// TestCopyMessageToClipboardOSC52 tests that pressing 'c' on a message
// sends OSC 52 to write to clipboard.
// NOTE: This test requires a message to be present. Since we can't easily
// create a mock LLM response, we test the copy keybinding behavior instead.
